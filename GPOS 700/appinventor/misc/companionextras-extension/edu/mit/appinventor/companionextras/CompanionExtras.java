...
@DesignerComponent(version = 1, category = ComponentCategory.EXTENSION, description = "An extension to provide additional functionality to non-Play Store Companions", nonVisible = true)
...
public class CompanionExtras extends AndroidNonvisibleComponent {
  ...
  private static final String REPL_ASSET_DIR = Environment.getExternalStorageDirectory().getAbsolutePath()
      + "/AppInventor/assets/";
    ...
    AsynchUtil.runAsynchronously(new Runnable() {
      ...
      public void run() {
        ...
        try {
          ...
          form.dispatchErrorOccurredEvent(form, "CompanionExtras", ErrorMessages.ERROR_UNABLE_TO_INSTALL_PACKAGE,
              packageuri);
        } catch (Exception e) {
          Log.e(LOG_TAG, "ERROR_UNABLE_TO_GET", e);
          form.dispatchErrorOccurredEvent(form, "CompanionExtras", ErrorMessages.ERROR_WEB_UNABLE_TO_GET, urlToApk);
        }
      }
    });
  }
