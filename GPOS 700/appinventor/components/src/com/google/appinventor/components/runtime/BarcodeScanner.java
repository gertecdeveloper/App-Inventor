 ...
 */
@DesignerComponent(version = YaVersion.BARCODESCANNER_COMPONENT_VERSION, description = "Component for using the Barcode Scanner to read a barcode", category = ComponentCategory.SENSORS, nonVisible = true, iconName = "images/barcodeScanner.png")
@SimpleObject
@UsesActivities(activities = {
    @ActivityElement(name = "com.google.zxing.client.android.AppInvCaptureActivity", screenOrientation = "landscape", stateNotNeeded = "true", configChanges = "orientation|keyboardHidden", theme = "@android:style/Theme.NoTitleBar.Fullscreen", windowSoftInputMode = "stateAlwaysHidden") })
...
public class BarcodeScanner extends AndroidNonvisibleComponent implements ActivityResultListener, Component {
  ...
  /*
   * Used to identify the call to startActivityForResult. Will be passed back into
   * the resultReturned() callback method.
   */
  ...
  public void DoScan() {
    ...
    if (!useExternalScanner && (SdkLevel.getLevel() >= SdkLevel.LEVEL_ECLAIR)) { // Should we attempt to use an internal
                                                                                 // scanner?
      // Make sure we have CAMERA permission
      if (!havePermission) {
        container.$form().askPermission(Manifest.permission.CAMERA, new PermissionResultHandler() {
          @Override
          public void HandlePermissionResponse(String permission, boolean granted) {
            if (granted) {
              ...
              form.dispatchPermissionDeniedEvent(BarcodeScanner.this, "DoScan", Manifest.permission.CAMERA);
            }
          }
        });
        return;
  ...
  /**
   * Gets whether or not you want to use an external scanning program to scan
   * barcodes.
   *
   * @return 'true' or 'false' depending on whether or not you want to use an
   *         external scanning program.
   */

  @SimpleProperty(category = PropertyCategory.BEHAVIOR, description = "If true App Inventor will look for and use an external scanning"
  ...
  /**
   * Set whether or not you wish to use an External Scanning program such as Bar
   * Code Scanner. If false a version of ZXing integrated into App Inventor will
   * be used.
   *
   * @param useExternalScanner Set true to use an external scanning program, false
   *                           to use internal copy of ZXing.
   *
   */
